...
upo_RRT::SimpleRRT::SimpleRRT() : Planner() {
	
	//maxRange_ = 0.5;
	//steering_ = new Steering(space_, maxRange_);
}

upo_RRT::SimpleRRT::~SimpleRRT() {

}



upo_RRT::State* upo_RRT::SimpleRRT::steer(State* fromState, State* toState, std::vector<State>& istates)
{
...
std::vector<upo_RRT::Node> upo_RRT::SimpleRRT::solve(float secs)
{
...
/*void upo_RRT::RRT::getTree(std::vector<upo_RRT::Node*> &tree) const {
	//nn_->list(tree);
	nn_->getTree(tree);
